...
| # | ID | Fix | Severity | Migration | Done |
|---|----|-----|----------|-----------|------|
| 6 | H1b | Build each series' bezier path once, fill and stroke from it | High | No | ➖ Dropped |
...
> **Phase 2 done (2026-08-13).** Builds clean, 501/501 tests pass.
> - **H1a:** `BuildPoints` moved to `Core/Charting/ChartGeometry.cs` — pure, no Win2D types, and covered by `ChartGeometryTests`. It now folds the series to one point per horizontal pixel column, keeping the column **maximum** so a one-second burst survives, and keeping the x of the sample that won the column so a spike stays where it happened. The live lead still reads the last complete bucket from the **source** series, not from the folded points, or the rightmost value would become a column maximum rather than a bucket.
> - **H1a is bounded by the widget's width**, which the finding above understated: 300 → ~170 points on the 170 DIP strip cell (~43% of the bezier segments), ~20% on the vertical widget at 240 wide, and deliberately nothing at all on the full-page charts, where 60 points into ~600 px never share a column.
> - **H1b: dropped, not deferred.** The finding assumed a shared path costs nothing visually. It does not. The fill figure has to close down to the baseline and back along it, and stroking that same geometry draws the baseline and both side edges with it; Win2D has no way to share the chain and stop at the curve. The alternatives were a clip that trims ~0.75 px off each end of the stroke — including the right edge where the live lead sits, the exact place the eye goes — or sharing only the control-point arithmetic, which is not where the cost is. H1c removes whole frames instead, which beats halving the cost of a frame that is no longer drawn.
> - **H1c:** `ShouldRedraw()` gates the `CompositionTarget.Rendering` invalidate on the scroll having moved at least half a pixel once the ease has settled to within a quarter pixel (values and axis maximum both). New data bypasses it entirely — `ApplyPoints` invalidates directly — so a flush always draws.
> - **The ease step is now clamped to 50 ms** (`MaxEaseStepSeconds`). Without it the first frame after a throttled second would apply a full second of ease in one step (factor 0.33 against the usual 0.006) and the post-flush ramp would snap rather than ease — the throttle would have broken the very thing it was preserving.
> - **Where the win lands:** an idle widget on the strip redraws about once a second rather than 60 or 144 times, because the window is 300 s wide in 170 px — 0.57 px/s. Under live traffic the ease is rarely settled, so the frame rate there is much as before and the saving is H1a's smaller geometry.
> - **Live check passed (2026-08-14).** Thirteen checks over both orientations at minimum size and dragged large: the idle trace still creeps rather than ticks; a burst after a quiet spell eases in and the axis maximum eases with it, so the `MaxEaseStepSeconds` clamp holds; the chart resumes the instant the pointer leaves a hover-freeze; the full-page charts are unchanged at the 5-minute range and stable on the long ranges, where the throttle is at its most aggressive (~0.007 px/s at 24 h, so roughly one redraw per 72 s); new data, palette changes, resize and orientation switches all still draw immediately, confirming the direct-`Invalidate` paths bypass the throttle as intended; and a single short burst is still visible as a spike at the decimated resolution.
> - **The frame saving is not visible in process CPU, and is recorded here as unproven.** The app measured **5% idle and 5% under sustained traffic** on the strip at minimum size. Two things follow. The first is that no pre-change baseline was captured, so there is no delta to quote — the figure records what the app costs now, not what it saved. The second is more interesting: if the throttle were the dominant term, idle would sit well below busy, and it does not. That points at a floor somewhere other than the chart — the ETW session, the scan loop, or the WinUI compositor's own present cycle — with the chart's frames too small a share of the total to show up. The reasoning behind H1c is still sound (a frame that cannot change a pixel is not worth drawing) and nothing regressed, but the "60 or 144 frames down to one" claim above describes the redraw count, not a measured CPU saving, and should not be read as one. Quantifying it needs a profiler on the render thread rather than Task Manager on the process.
...
- **Phase 4 (chart buffers).** ✅ Done 2026-08-18. All seven checks passed. Both full-page charts scroll smoothly with `ChartSmoothScrolling` on, with no stutter and no ghost of the previous window; range switches in both directions — 5m → 1h → 24h and back — carry over cleanly, which is the case that forces the reused buffers to fall back to fresh arrays because the point count changes; an emptied window refills from nothing correctly; both widget orientations and both sections draw as before; `ChartSmoothScrolling` off and on both still draw; the Local device lens shows device names, which is what exercises L3's new EF projection; and a cold start from the Windows Startup task starts, scans and shows sensible scan settings, which is the launch path L1's base-path change was aimed at and the one Visual Studio cannot reproduce.
